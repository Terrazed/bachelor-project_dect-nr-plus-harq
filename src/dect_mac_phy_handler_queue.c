...
int dect_phy_queue_put(enum dect_mac_phy_function function, union dect_mac_phy_handler_params *params, uint32_t priority)
{
    ...
    k_mutex_lock(&queue_mutex, K_FOREVER);
    {
        ...
        if(sys_slist_len(&dect_mac_phy_handler_queue) >= DECT_MAC_PHY_HANDLER_QUEUE_MAX_ITEMS)
        { // warning! this operation is O(n) which means that if the queue max item is big it should probably be handled differently
        ...
        else
        {
            ...
            for(sys_snode_t *node = sys_slist_peek_head(&dect_mac_phy_handler_queue); // create a node to iterate through the list
                ...
                previous_node = node, node = sys_slist_peek_next(node))
            { // set the previous node to the current node and the current node to the next node
